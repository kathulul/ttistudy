"""script to process extra chatlogs that were emailed to me. this will go into ignored scripts later"""
...
# for SPECIFICALLY the chatgpt pattern 
UNWANTED_PATTERNS = [
...
def log_presidio_detections(text: str, confidence_threshold: float = 0.8) -> tuple[str, dict]:
    ...
    try:
        ...
        # create dictionary to store entities
        ...
        # sort results by confidence (its kind of broken i think presidio has a set confidence threshold.)
...
def clean_chatlog(chatlog: str, detected_entities: dict, confidence_threshold: float = 0.8, progress_callback=None) -> str:
    ...
    try:
        ...
        cleaned_text = chatlog.strip()  # Remove whitespace
        cleaned_text = re.sub(r'\r\n?', '\n', cleaned_text) # Normalize line ending
        
        # Remove cached patterns
        ...
        # Replace all instances of detected entities with [ENTITY]
        for entity_type, words in detected_entities.items():
            for word in words:
                # matches whole word case-insensitively
...
def process_chatlogs(chatlog_path: str):
    """
    Process a single chatlog file to generate presidio detection log and cleaned chatlog.
    ...
    try:
        print("\n=== Starting Processing ===")
        print(f"Reading file: {chatlog_path}")
        
        # Read the chatlog content
        with open(chatlog_path, 'r') as chatlog_file:
            chatlog_content = chatlog_file.read().strip()
            print(f"\nOriginal content length: {len(chatlog_content)} characters")
            print("First 100 chars of original content:")
            print(chatlog_content[:100] + "...")
            
            # Skip empty chatlogs
            if not chatlog_content:
                print(f"Warning: Empty chatlog found at {chatlog_path}")
                return
            
            print("\n=== Running Presidio Detection ===")
            ...
            # Save detection log in the same directory as the input file
            detection_log_path = os.path.join(os.path.dirname(chatlog_path), 'presidio_detections.txt')
            print(f"\nSaving detection log to: {detection_log_path}")
            ...
            print(f"Cleaned content length: {len(cleaned_chatlog)} characters")
            print("First 100 chars of cleaned content:")
            print(cleaned_chatlog[:100] + "...")
            
            # Ensure we have content to save
            if not cleaned_chatlog.strip():
                print("Warning: Cleaned chatlog is empty after processing")
                return
                
            # Save cleaned chatlog in the same directory as the input file
            cleaned_chatlog_path = os.path.join(os.path.dirname(chatlog_path), 'cleaned_chatlog.txt')
            print(f"\nSaving cleaned chatlog to: {cleaned_chatlog_path}")
            
            # Ensure the directory exists
            os.makedirs(os.path.dirname(cleaned_chatlog_path), exist_ok=True)
            ...
            if os.path.exists(cleaned_chatlog_path):
                with open(cleaned_chatlog_path, 'r', encoding='utf-8') as verify_file:
                    saved_content = verify_file.read()
                    print(f"\nVerified saved content length: {len(saved_content)} characters")
            
            print("\n=== Processing Complete ===")
                
    except Exception as e:
        print(f"\nError processing chatlog at {chatlog_path}: {str(e)}")
        import traceback
        traceback.print_exc()

if __name__ == "__main__":
    process_chatlogs("chatlogsD/24/raw_chatlog.txt")
